...
@interface NSArray (XCategory)

/*
 * Calls performSelector on all objects that can receive the selector in the array.
 * Makes an iterable copy of the array, making it possible for the selector the modify the array.
 * Contrast this with makeObjectsPerformSelector which does not allow side effects of modifying the array.
 */
...
// Extensions to makeObjectsPerformSelector to provide support for more than one object parameter.
...
// @return nil or an object that matches values value with isEqual
